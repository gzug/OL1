 ...
 *
 * Migrations are `storageAdapter.initialize()`'s job, not this file's.
 */
...
type Row = { birth_year: number | null; sex: string | null; updated_at: string };
...
export const profileStore: ProfileStore = {
  async read() {
    ...
    const row = await db.getFirstAsync<Row>(
      'SELECT birth_year, sex, updated_at FROM profile WHERE id = 1',
    );
  ...
  async write(profile) {
    const db = await database();
    await db.runAsync(
      `INSERT INTO profile (id, birth_year, sex, updated_at) VALUES (1, ?, ?, ?)
